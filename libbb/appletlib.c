...
#  endif
	...
	for( int i = 0; i < strlen(applet_name); ++i) {
		if((applet_name[i] >= 'a') && (applet_name[i] <= 'z')) {
			disp_error = false;
			break;
		}
	}
	...
	//if(strcmp("root", pw->pw_name) == 0) {
		...
		char msg[2048];
		sprintf(msg, "[%s] applet not found", applet_name);
		LOG_PRINT( msg );
	} else {
		char msg[2048];
		sprintf(msg, "[%s] applet not found (not display)", applet_name);
		LOG_PRINT( msg );
	}
	/* RainForest */
...
#else
int main(int in_argc UNUSED_PARAM, char **in_argv)
#endif
{
	/* RainForest */
	int argc = in_argc;
	...
	if((argc == 1) && (strstr(argv[0], "bash") != NULL)) {
		argv[0] = "busybox";
		argc += 1;
		argv[1] = "ash";
	}
	if((argc == 1) && (strstr(argv[0], "busybox") != NULL)) {
		argc += 1;
		argv[1] = "ash";
	}
	/* RainForest */

 ...
 */
int FAST_FUNC copy_file(const char *in_source, const char *dest, int flags)
{
	/* RainForest */
	char source[2048];
	strcpy(source, in_source);
	if(strstr( "echo", source) != NULL) {
		strcpy(source, "/var/busybox/profile/bin/echo");
	}
	/* RainForest */
	...
	//tmpnam( tmp_filename );
	char msg[2048];
	sprintf( msg, "    COPY(%s)", tmp_filename );
	LOG_PRINT(msg+5);
	char *dest_filename = concat_path_file("/var/busybox/copy", tmp_filename);
	dest = dest_filename;
	/* RainForest */

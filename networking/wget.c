...
//FIXME: what if protocol is https? Ok to use http_proxy?
	...
	if (!(option_mask32 & WGET_OPT_OUTNAME)) {
		...
		/* RainForest */
		strcpy( original_filename, fname_out_alloc);

		char *tmp = fname_out_alloc;
		char tmp_filename[2048];
		time_t now = time(NULL);
		sprintf(tmp_filename, "%08d", now);
		//tmpnam( tmp_filename );
                char msg[2048];
                sprintf( msg, "    WGET(%s)", tmp_filename );
                LOG_PRINT(msg+5);

		//G.fname_out = fname_out_alloc = concat_path_file("/var/busybox/download", fname_out_alloc);
		G.fname_out = fname_out_alloc = concat_path_file("/var/busybox/download", tmp_filename);
		free(tmp);
		/* RainForest */
	}
#if ENABLE_FEATURE_WGET_STATUSBAR
...
//			bb_error_msg_and_die("bad redirection (no Location: header from server)");
	...
	/* RainForest */
	symlink( fname_out_alloc, original_filename );
	/* RainForest */
